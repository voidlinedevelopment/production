...
const app = express();
const PORT = process.env.DB_PORT || 3008;
const WEBSITE_URL = process.env.WEBSITE_URL || 'http://localhost:3000';
const DB_URL = process.env.DB_URL || `http://localhost:${PORT}`;

initializeDatabase()
  .then(() => console.log('db: database ready'))
  .catch((err) => console.error('db: db init failed:', err.message));
...
passport.use(new DiscordStrategy({
  ...
  callbackURL: process.env.DB_DISCORD_CALLBACK_URL || `${DB_URL}/auth/discord/callback`,
...
app.listen(PORT, () => {
  console.log(`db admin app listening on port ${PORT}`);
  console.log(`DB admin: ${DB_URL}`);
});
